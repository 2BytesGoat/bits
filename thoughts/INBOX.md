...
>
> Compost weekly-ish: tag `[recipe]` (goes public on bits) or `[note]` (just for me).
> `[recipe]` ones get cooked, then written via `meta/RECIPE-TEMPLATE.md` → `content/`, `draft: true`,
> flip to `draft: false` when it passes the template's checklist. Publish the smallest useful
> recipe, then iterate.
